...
function GpsIntegrationPage() {
  ...
  const kpis = useMemo(() => {
    ...
    const avgSpeed = total ? (imported.reduce((a,b)=>a+b.velocidad,0)/total).toFixed(1) : 0;
    return { total, moving, stopped, avgSpeed };
  }, [imported]);

  const validateFile = () => {
    let errs = [];
    if (REQUIRED_COLUMNS.length !== 8) errs.push("Estructura inválida");
    setErrors(errs);
  ...
  const importData = () => {
    ...
    const rows = [
      { fecha:"2026-05-04", hora:"10:00", placa:"ABC123", latitud:-12.04, longitud:-77.03, velocidad:60, rumbo:120, distancia_total:10, proveedor: provider },
      { fecha:"2026-05-04", hora:"10:00", placa:"ABC123", latitud:-12.04, longitud:-77.03, velocidad:60, rumbo:120, distancia_total:10, proveedor: provider }
    ];
  ...
  const downloadTemplate = () => {
    const blob = new Blob([REQUIRED_COLUMNS.join(",")], { type: "text/csv" });
  ...
  const filteredData = imported.filter(d =>
    (filterProvider === "all" || normalize(d.proveedor) === normalize(filterProvider)) &&
  ...
  return (
    <DashboardLayout>
      ...
      <div className="panel">
        ...
        <button className={`btn ${isValid ? "purple" : "disabled"}`} disabled={!isValid} onClick={importData}>
          Importar Datos
        </button>
      </div>
      ...
      <div className="panel">
        ...
        <div className="filters">
          ...
          <input placeholder="Buscar placa" onChange={e => setFilterPlate(e.target.value)} />
        </div>
